...
def get_outlook_status():
    return {"status": "connected", "provider": "outlook", "message": "Outlook Calendar Integration Core Active"}

@router.post("/token", response_model=CalendarTokenResponse)
def save_outlook_token(token_in: CalendarTokenCreate, db: Session = Depends(get_db)):
    db_token = db.query(CalendarToken).filter(
        CalendarToken.user_id == token_in.user_id,
        CalendarToken.provider == "outlook"
    ).first()
...
def refresh_outlook_access_token(user_id: int, db: Session) -> Optional[str]:
    db_token = db.query(CalendarToken).filter(
    ...
    if client_id == "MOCK_MICROSOFT_CLIENT_ID" or client_secret == "MOCK_MICROSOFT_SECRET":
        print("Using mock Microsoft OAuth credentials. Bypassing live refresh.")
        return None
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Failed to refresh Microsoft token: {response.text}")
            return None
    except Exception as e:
        print(f"Error during Microsoft token refresh: {e}")
        return None

@router.post("/refresh/{user_id}")
def refresh_token_endpoint(user_id: int, db: Session = Depends(get_db)):
    token = refresh_outlook_access_token(user_id, db)
    if not token:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Could not refresh Outlook token or no token exists"
        )
    return {"access_token": token, "status": "refreshed"}

def create_outlook_calendar_event(user_id: int, title: str, description: Optional[str], scheduled_time: datetime, db: Session):
    access_token = refresh_outlook_access_token(user_id, db)
    if not access_token:
        print(f"No active Microsoft Calendar token found for user_id={user_id}. Skipping event creation.")
        return None
    ...
    payload = {
        "subject": f"💪 Muscleki: {title}",
        "body": {
            "contentType": "HTML",
            "content": description or "Scheduled from Muscleki AI Scheduler"
        },
        "start": {
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            print(f"Microsoft Calendar event creation failed: {response.text}")
            return None
    except Exception as e:
...
def create_event_endpoint(event_in: CalendarEventCreate, db: Session = Depends(get_db)):
    result = create_outlook_calendar_event(
    ...
    if not result:
        return {
            "status": "simulated",
            "message": "Real Microsoft Calendar integration skipped (OAuth token or API credentials not configured). Registered in DB.",
            "title": event_in.title
        }
